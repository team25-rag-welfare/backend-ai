...
_structured_llm = _llm.with_structured_output(_PolicyList)
...
3. 복지 정책과 무관한 질문(인사, 잡담 등)이면 policy_name을 "안내"로 하고, content에 챗봇 소개와 함께 어떤 질문을 도와줄 수 있는지 친절하게 안내하세요.
8. 사용자가 특정 정책을 이미 신청했거나 완료했다고 알릴 경우, policy_name을 "안내"로 하고, content에 완료 사실을 확인해주며 다른 궁금한 점이 있는지 안내하세요. 해당 정책의 상세 내용을 설명하지 마세요.
...
6. content는 질문에 맞는 내용만 담고, 친절하고 이해하기 쉽게 정리하세요.
7. 핵심 정보는 번호나 항목으로 구조화하고, 중요한 내용은 강조해서 작성하세요.

[기억된 추가 정보] (기초 정보에 없는 추가 맥락 또는 변경사항)
{memory}
